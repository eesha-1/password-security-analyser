An application designed to analyse password strength and security. Built using Python, Flask, HTML and CSS. Deployed using Render. 

Key features implemented:
• Password strength scoring
• Password length checking
• Uppercase and lowecase detection
• Symbol detection (e.g. -, _, *, ')
• Number detection 
• Recommended secure password generation
• Responsive frontend design for mobile and desktop devices

Website live link: https://password-security-analyser-klol.onrender.com/


Author: Eesha Mohammed.